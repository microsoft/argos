# Mutation: Added a new normal and abnormal rule for detecting outliers based on Z-score
...
def inference(sample: np.ndarray) -> np.ndarray:
    ...
    # Normal Rule 4: Data should not contain outliers based on Z-score (>3 or < -3)
    ...
    # Abnormal Rule 4: Detect outliers using Z-score
    z_scores = np.abs((sample[:, 0] - mean) / std_dev)
